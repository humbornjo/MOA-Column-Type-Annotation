...
if __name__ == '__main__':
    # kg module prep
    with open('./data/fine-grained_WikiTables.pkl', 'rb') as f:
        kg_df = pickle.load(f)
    with open('./data/mapping_dict.json', 'r', encoding='utf-8') as f:
        kg_map_dict = json.load(f)
    kg_tokenizer = AutoTokenizer.from_pretrained("princeton-nlp/sup-simcse-bert-base-uncased")
    kg_model = AutoModel.from_pretrained("princeton-nlp/sup-simcse-bert-base-uncased")
    fb = sorted(kg_map_dict.items(), key = lambda x:x[1])
    fb_sorted_list = [(i[0].split('.')[1]+' of '+i[0].split('.')[0]).replace('_', ' ') for i in fb]
    tokenized_fb_types = kg_tokenizer(fb_sorted_list, padding=True, truncation=True, return_tensors="pt")
    with torch.no_grad():
        fb_embeddings = kg_model(**tokenized_fb_types, output_hidden_states=True, return_dict=True).pooler_output
    ts = 0
    count = 0
    alpha = 0.3
    beta = 1-alpha
...
#########################################################################################
    ...
    for f1_name, model_path in [("f1_macro", f1_macro_model_path),
        ...
        for batch_idx, batch in enumerate(test_dataloader):
            ...
            #     ).tolist()
            print(len(type_seq_list),len(header_seq_list), cls_indexes.shape[0],'\n')
            for n in range(cls_indexes.shape[0]):
                cleaned_type = {}
                for key in type_seq_list[n].keys():
                    if type_seq_list[n][key]>1:
                        cleaned_type[key] = type_seq_list[n][key]
                complex_list = sorted(cleaned_type.items(), key = lambda x:x[1], reverse=True)
                ...
                if target_list:
                    ...
                    es = np.array(count_list).dot(Z_Score_Normalization(es_cal(kg_model, target_list, fb_embeddings)))
                
                hd_ls = ls_cal([header_seq_list[n]], fb_sorted_list)
                hd_es = es_cal(kg_model, [header_seq_list[n]], fb_embeddings)
    
                i, j = cls_indexes[n]
                logit_n = logits[i, j, :]
                
                # print(logit_n.detach().cpu().numpy().shape, ls.shape)
                # filtered_logits[n] = (logit_n >= math.log(0.5))  #>= logit_n.max()
                conf = (Z_Score_Normalization(logit_n.detach().cpu().numpy()) + \
                        softmax(ls)*alpha + softmax(es)*beta + \
                        (softmax(hd_ls)*alpha + softmax(hd_es)*beta)*gamma)
                filtered_logits[n] = (conf >= np.max(conf))
