...
from util import cl_loss, cl_loss_ce
...
from dataset import (
    ...
    LpnembedSatoCVTablewiseDataset,
    LpnembedTURLColTypeColwiseDataset,
    SatoCVColwiseDatasetForCl
)

from model import BertForMultiOutputClassification, BertMultiPairPooler, BertForSequenceClassification, BertClassificationModel, BertModelMultiOutput
...
if __name__ == "__main__":
    ...
    parser.add_argument("--cl",
                        default=False,
                        action="store_true",
                        help="using the embed method of Lpn")
    ...
    for i, num_classes in enumerate(num_classes_list):
        if args.single_col:
            ...
            # model = BertClassificationModel(type_numbers=num_classes)
        else:
            if args.from_scratch:
    ...
    if args.cl: 
        cl_model = BertModelMultiOutput.from_pretrained(shortcut_name)
        cl_model.embeddings=models[0].bert.embeddings
        cl_model.encoder=models[0].bert.encoder
        cl_model.to(device)
        pre_cl_dataset = SatoCVColwiseDataset(cv='4',
                                    split="train",
                                    tokenizer=tokenizer,
                                    max_length=max_length,
                                    multicol_only=False,
                                    train_ratio=train_ratio,
                                    device=device)
        cl_optimizer = AdamW(params = cl_model.parameters(), lr=5e-5, eps=1e-8)
        cl_scheduler = get_linear_schedule_with_warmup(cl_optimizer,
                                                        num_warmup_steps=0,
                                                        num_training_steps=20)

        cl_dataset = SatoCVColwiseDatasetForCl(df=pre_cl_dataset.df,
                                                cm_analysis_dir="/home/shensy/Code/python/doduo/eval/sato4_mosato_bert_bert-base-uncased-bs16-ml-32__sato4-1.00=sato4.json",
                                                num_data=5000,
                                                device=device)
        # TODO: coding dataloader
        cl_dataloader = DataLoader(cl_dataset,
                                          batch_size=batch_size,
                                          collate_fn=collate_fn)
        cl_loss_info_list = []

        models.append(cl_model)
        optimizers.append(cl_optimizer)
        schedulers.append(cl_scheduler)
        # Store dataloaders
        train_datasets.append(cl_dataset)
        train_dataloaders.append(cl_dataloader)
        valid_datasets.append(None)
        valid_dataloaders.append(None)
        tasks = args.tasks.append('cl')
        loss_fns.append(cl_loss_ce)
        loss_info_lists.append([])

    
    print("===start training===")
    
    # I want to add contrastive learning before the doduo training
    # the neg samples are sampled based on the eva result
    # 221125 only for sato dataset
        ...
        for k, (task, model, train_dataset, valid_dataset, train_dataloader,
            ...
            if task == 'cl':
                ...
                for batch_idx, batch in enumerate(train_dataloader):
                    model.train()
                    input = torch.reshape(batch["data"].permute([1,2,0]),(-1, batch["data"].shape[0]))
                    outputs = cl_model(input)
                    pooled_output = outputs[1]  # (N, 768)
                    # print(pooled_output.shape)
                    # pooled_output = torch.mean(pooled_output, dim = 1)             
                    pooled_output = pooled_output[:,0,:]             
                    # print(pooled_output.shape, batch["label"].shape)
                    # TODO: coding loss fct
                    logits = torch.reshape(pooled_output, (-1,8,768))
                    loss = cl_loss_ce(logits, batch["label"])
                    cl_tr_loss+=loss
                    cl_loss_info_list.append(loss)
                    loss.backward()
                    optimizer.step()
                    scheduler.step()
                    model.zero_grad()
                cl_t2 = time()
                print(
                    "Epoch {} ({}): cl_loss={:.7f} ({:.2f} sec.)"
                    .format(epoch, "cl", cl_tr_loss, (cl_t2 - cl_t1)))
    ...
    for task, loss_info_list in zip(args.tasks, loss_info_lists[:-1]):
